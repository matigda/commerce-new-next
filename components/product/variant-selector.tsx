...
type OptimizedVariant = {
  ...
  [key: string]: string | number | boolean | URLSearchParams; // ie. { color: 'Red', size: 'Large', ... }
};
...
export function VariantSelector({
  ...
  const optimizedVariants: OptimizedVariant[] = variants.map((variant) => {
    ...
    variant.pickedOptions?.forEach((selectedOption) => {
      const name = selectedOption.name.toLowerCase();
      const value = selectedOption.value;

      optimized[name] = value.toString();
      optimized.params.set(name, value.toString());
    });
  ...
  return options.map((option) => (
    <dl className="mb-8" key={Math.random()}>
            ...
            <DynamicTag
              ...
              onClick={() => {
                setChoices((currentChoices: PickedOption[]) => {
                  const newChoice = currentChoices.find((el) => el.name === option.name);
                  newChoice!.value = value;
                  const newChoices = [
                    ...currentChoices.filter((el) => el.name !== option.name),
                    newChoice
                  ] as PickedOption[];
